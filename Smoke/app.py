    ...
    while True:
        ...
        if results[0].boxes is not None and results[0].boxes.id is not None:
            ...
            for box, class_id, track_id in zip(boxes, class_ids, track_ids):
                ...
                color = (0, 255, 0) if (label.lower() != "cigarette" or label.lower() != "smoke") else (0, 0, 255)
                cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
                cv2.putText(frame, f'{track_id} - {label}', (x1, y1 - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)

                # Check for "No Queue" class (Assume class 1 = "No Queue")
                if (label.lower() != "cigarette" or label.lower() != "smoke") and track_id not in recent_detections:
